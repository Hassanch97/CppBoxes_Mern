...
const headCells = [
    { id: 'id', label: 'ID' },
    { id: 'old_url', label: 'Old URL' },
    { id: 'new_url', label: 'New URL' },
...
const Redirections = () => {
    ...
    React.useEffect(() => {
        fetch('http://localhost:3000/redirections/')
        .then((res) => res.json())
        .then((data) => {
        ...
        .catch((err) => {
        console.error('Error fetching redirections:', err);
        setRows([]);
        });
    }, []);
    const filteredRows = React.useMemo(() => {
        ...
        return rows.filter(
        (row) =>
        (row.old_url && row.old_url.toLowerCase().includes(lowerQuery)) ||
        (row.new_url && row.new_url.toLowerCase().includes(lowerQuery))
        );
    }, [rows, searchQuery]);
  ...
  return (
    <div>
        ...
        <div className="catergories_main">
            ...
            <Button variant="contained">Add Redirection</Button>
        </div>
        <Box sx={{ width: '100%' }}>
                    ...
                    <Table sx={{ minWidth: 750 }} aria-labelledby="tableTitle" size={dense ? 'small' : 'medium'}>
                        ...
                        <TableBody>
                            ...
                            return (
                                ...
                                <TableCell>{row.old_url}</TableCell>
                                <TableCell>{row.new_url}</TableCell>
